...
{
	ASoldierAIController* MyController = Cast<ASoldierAIController>(InActor);
				...
				if (Hit.GetActor() != NULL)
				{
					...
					else
					{
							...
							&& (MyController->PlayerState != NULL) && (HitChar->GetPlayerState() != NULL))
						{
							...
							ASoldierPlayerState* MyPlayerState = Cast<ASoldierPlayerState>(MyController->PlayerState);
