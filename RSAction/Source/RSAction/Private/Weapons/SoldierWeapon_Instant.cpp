...
ASoldierWeapon_Instant::ASoldierWeapon_Instant(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
...
void ASoldierWeapon_Instant::FireWeapon()
{
...
bool ASoldierWeapon_Instant::ServerNotifyHit_Validate(const FHitResult& Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void ASoldierWeapon_Instant::ServerNotifyHit_Implementation(const FHitResult& Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
bool ASoldierWeapon_Instant::ServerNotifyMiss_Validate(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void ASoldierWeapon_Instant::ServerNotifyMiss_Implementation(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	const FVector Origin = GetMuzzleLocation();
...
void ASoldierWeapon_Instant::ProcessInstantHit(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
void ASoldierWeapon_Instant::ProcessInstantHit_Confirmed(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
	// handle damage
...
bool ASoldierWeapon_Instant::ShouldDealDamage(AActor* TestActor) const
{
...
void ASoldierWeapon_Instant::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
...
void ASoldierWeapon_Instant::OnBurstFinished()
{
...
float ASoldierWeapon_Instant::GetCurrentSpread() const
{
...
void ASoldierWeapon_Instant::OnRep_HitNotify()
{
	SimulateInstantHit(HitNotify.Origin, HitNotify.RandomSeed, HitNotify.ReticleSpread);
}

void ASoldierWeapon_Instant::SimulateInstantHit(const FVector& ShotOrigin, int32 RandomSeed, float ReticleSpread)
{
	FRandomStream WeaponRandomStream(RandomSeed);
...
void ASoldierWeapon_Instant::SpawnImpactEffects(const FHitResult& Impact)
{
	if (ImpactTemplate && Impact.bBlockingHit)
	{
		...
		ASoldierImpactEffect* EffectActor = GetWorld()->SpawnActorDeferred<ASoldierImpactEffect>(ImpactTemplate, SpawnTransform);
		if (EffectActor)
		{
...
void ASoldierWeapon_Instant::SpawnTrailEffect(const FVector& EndPoint)
{
	if (TrailFX)
	{
		const FVector Origin = GetMuzzleLocation();
...
void ASoldierWeapon_Instant::GetLifetimeReplicatedProps( TArray< FLifetimeProperty > & OutLifetimeProps ) const
{
	Super::GetLifetimeReplicatedProps( OutLifetimeProps );

	DOREPLIFETIME_CONDITION( ASoldierWeapon_Instant, HitNotify, COND_SkipOwner );
}