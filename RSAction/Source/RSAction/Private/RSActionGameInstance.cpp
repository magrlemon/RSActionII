...
namespace RSActionGameInstanceState
{
...
URSActionGameInstance::URSActionGameInstance(const FObjectInitializer& ObjectInitializer)
...
{
	CurrentState = RSActionGameInstanceState::None;
}

void URSActionGameInstance::Init() 
{
	Super::Init();
	...
	{
		IdentityInterface->AddOnLoginStatusChangedDelegate_Handle(i, FOnLoginStatusChangedDelegate::CreateUObject(this, &URSActionGameInstance::HandleUserLoginChanged));
	}

	IdentityInterface->AddOnControllerPairingChangedDelegate_Handle(FOnControllerPairingChangedDelegate::CreateUObject(this, &URSActionGameInstance::HandleControllerPairingChanged));

	FCoreDelegates::ApplicationWillDeactivateDelegate.AddUObject(this, &URSActionGameInstance::HandleAppWillDeactivate);

	FCoreDelegates::ApplicationWillEnterBackgroundDelegate.AddUObject(this, &URSActionGameInstance::HandleAppSuspend);
	FCoreDelegates::ApplicationHasEnteredForegroundDelegate.AddUObject(this, &URSActionGameInstance::HandleAppResume);

	FCoreDelegates::OnSafeFrameChangedEvent.AddUObject(this, &URSActionGameInstance::HandleSafeFrameChanged);
	FCoreDelegates::OnControllerConnectionChange.AddUObject(this, &URSActionGameInstance::HandleControllerConnectionChange);
	FCoreDelegates::ApplicationLicenseChange.AddUObject(this, &URSActionGameInstance::HandleAppLicenseUpdate);

	FCoreUObjectDelegates::PreLoadMap.AddUObject(this, &URSActionGameInstance::OnPreLoadMap);
	FCoreUObjectDelegates::PostLoadMapWithWorld.AddUObject(this, &URSActionGameInstance::OnPostLoadMap);

	FCoreUObjectDelegates::PostDemoPlay.AddUObject(this, &URSActionGameInstance::OnPostDemoPlay);

	bPendingEnableSplitscreen = false;

	OnlineSub->AddOnConnectionStatusChangedDelegate_Handle( FOnConnectionStatusChangedDelegate::CreateUObject( this, &URSActionGameInstance::HandleNetworkConnectionStatusChanged ) );

	if (SessionInterface.IsValid())
	{
		SessionInterface->AddOnSessionFailureDelegate_Handle( FOnSessionFailureDelegate::CreateUObject( this, &URSActionGameInstance::HandleSessionFailure ) );
	}
	
	OnEndSessionCompleteDelegate = FOnEndSessionCompleteDelegate::CreateUObject(this, &URSActionGameInstance::OnEndSessionComplete);

	// Register delegate for ticker callback
	TickDelegate = FTickerDelegate::CreateUObject(this, &URSActionGameInstance::Tick);
...
void URSActionGameInstance::Shutdown()
{
	Super::Shutdown();
...
void URSActionGameInstance::HandleNetworkConnectionStatusChanged( const FString& ServiceName, EOnlineServerConnectionStatus::Type LastConnectionStatus, EOnlineServerConnectionStatus::Type ConnectionStatus )
{
	UE_LOG( LogOnlineGame, Log, TEXT( "URSActionGameInstance::HandleNetworkConnectionStatusChanged: %s" ), EOnlineServerConnectionStatus::ToString( ConnectionStatus ) );
	...
	if (	OnlineMode != EOnlineMode::Offline && 
			PendingState != RSActionGameInstanceState::WelcomeScreen &&
			CurrentState != RSActionGameInstanceState::WelcomeScreen && 
			ConnectionStatus != EOnlineServerConnectionStatus::Connected &&
			ConnectionStatus != EOnlineServerConnectionStatus::Normal)
	{
		UE_LOG( LogOnlineGame, Log, TEXT( "URSActionGameInstance::HandleNetworkConnectionStatusChanged: Going to main menu" ) );
		...
		ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), RSActionGameInstanceState::MainMenu );
	}
...
void URSActionGameInstance::HandleSessionFailure( const FUniqueNetId& NetId, ESessionFailure::Type FailureType )
{
	UE_LOG( LogOnlineGame, Warning, TEXT( "URSActionGameInstance::HandleSessionFailure: %u" ), (uint32)FailureType );
	...
	if (	OnlineMode != EOnlineMode::Offline &&
			...
			CurrentState != RSActionGameInstanceState::WelcomeScreen )
	{
		UE_LOG( LogOnlineGame, Log, TEXT( "URSActionGameInstance::HandleSessionFailure: Going to main menu" ) );
...
void URSActionGameInstance::OnPreLoadMap(const FString& MapName)
{
...
void URSActionGameInstance::OnPostLoadMap(UWorld*)
{
...
void URSActionGameInstance::OnUserCanPlayInvite(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
	...
	else
	{
		...
		GotoState(RSActionGameInstanceState::WelcomeScreen);
	}
}

void URSActionGameInstance::OnUserCanPlayTogether(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{
	CleanupOnlinePrivilegeTask();
	if (WelcomeMenuUI.IsValid())
	{
...
void URSActionGameInstance::OnPostDemoPlay()
{
	GotoState( RSActionGameInstanceState::Playing );
}

void URSActionGameInstance::HandleDemoPlaybackFailure( EDemoPlayFailure::Type FailureType, const FString& ErrorString )
{
	...
	ShowMessageThenGotoState(FText::Format(NSLOCTEXT("URSActionGameInstance", "DemoPlaybackFailedFmt", "Demo playback failed: {0}"), FText::FromString(ErrorString)), NSLOCTEXT("DialogButtons", "OKAY", "OK"), FText::GetEmpty(), RSActionGameInstanceState::MainMenu);
}

void URSActionGameInstance::StartGameInstance()
{
#if PLATFORM_PS4 == 0
...
FGameInstancePIEResult URSActionGameInstance::StartPlayInEditorGameInstance(ULocalPlayer* LocalPlayer, const FGameInstancePIEParameters& Params)
{
...
#endif	// SOLDIER_CONSOLE_UI
	if (CurrentMapName == MainMenuMap)
	{
		GotoState(RSActionGameInstanceState::MainMenu);
	}
	else
	{
		GotoState(RSActionGameInstanceState::Playing);
...
FName URSActionGameInstance::GetInitialState()
{
#if SOLDIER_CONSOLE_UI	
	// Start in the welcome screen state on consoles
	return RSActionGameInstanceState::WelcomeScreen;
#else
	// On PC, go directly to the main menu
	return RSActionGameInstanceState::MainMenu;
#endif
}

void URSActionGameInstance::GotoInitialState()
{
	GotoState(GetInitialState());
}

const FName URSActionGameInstance::GetCurrentState() const 
{
	return CurrentState;
}

void URSActionGameInstance::ShowMessageThenGotoState( const FText& Message, const FText& OKButtonString, const FText& CancelButtonString, const FName& NewState, const bool OverrideExisting, TWeakObjectPtr< ULocalPlayer > PlayerOwner )
{
	UE_LOG( LogOnline, Log, TEXT( "ShowMessageThenGotoState: Message: %s, NewState: %s" ), *Message.ToString(), *NewState.ToString() );

	const bool bAtWelcomeScreen = PendingState == RSActionGameInstanceState::WelcomeScreen || CurrentState == RSActionGameInstanceState::WelcomeScreen;
	...
	const bool bAlreadyAtMessageMenu = PendingState == RSActionGameInstanceState::MessageMenu || CurrentState == RSActionGameInstanceState::MessageMenu;
	...
	if ( bAlreadyAtMessageMenu && PendingMessage.NextState == RSActionGameInstanceState::WelcomeScreen )
	{
	...
	if ( CurrentState == RSActionGameInstanceState::MessageMenu )
	{
	...
	{
		GotoState(RSActionGameInstanceState::MessageMenu);
	}
}

void URSActionGameInstance::ShowLoadingScreen()
{
...
bool URSActionGameInstance::LoadFrontEndMap(const FString& MapName)
{
	bool bSuccess = true;
	...
	if (URL.Valid && !HasAnyFlags(RF_ClassDefaultObject)) //CastChecked<UEngine>() will fail if using Default__RSActionGameInstance, so make sure that we're not default
	{
...
ASoldierGameSession* URSActionGameInstance::GetGameSession() const
{
	UWorld* const World = GetWorld();
	if (World)
	{
...
void URSActionGameInstance::TravelLocalSessionFailure(UWorld *World, ETravelFailure::Type FailureType, const FString& ReasonString)
{
...
void URSActionGameInstance::ShowMessageThenGoMain(const FText& Message, const FText& OKButtonString, const FText& CancelButtonString)
{
	ShowMessageThenGotoState(Message, OKButtonString, CancelButtonString, RSActionGameInstanceState::MainMenu);
}

void URSActionGameInstance::SetPendingInvite(const FSoldierPendingInvite& InPendingInvite)
{
	PendingInvite = InPendingInvite;
}

void URSActionGameInstance::GotoState(FName NewState)
{
...
void URSActionGameInstance::MaybeChangeState()
{
	if ( (PendingState != CurrentState) && (PendingState != RSActionGameInstanceState::None) )
	{
		...
		PendingState = RSActionGameInstanceState::None;
	}
}

void URSActionGameInstance::EndCurrentState(FName NextState)
{
	// per-state custom ending code here
	if (CurrentState == RSActionGameInstanceState::PendingInvite)
	{
		EndPendingInviteState();
	}
	else if (CurrentState == RSActionGameInstanceState::WelcomeScreen)
	{
		EndWelcomeScreenState();
	}
	else if (CurrentState == RSActionGameInstanceState::MainMenu)
	{
		EndMainMenuState();
	}
	else if (CurrentState == RSActionGameInstanceState::MessageMenu)
	{
		EndMessageMenuState();
	}
	else if (CurrentState == RSActionGameInstanceState::Playing)
	{
		EndPlayingState();
	}
...
void URSActionGameInstance::BeginNewState(FName NewState, FName PrevState)
{
	// per-state custom starting code here

	if (NewState == RSActionGameInstanceState::PendingInvite)
	{
		BeginPendingInviteState();
	}
	else if (NewState == RSActionGameInstanceState::WelcomeScreen)
	{
		BeginWelcomeScreenState();
	}
	else if (NewState == RSActionGameInstanceState::MainMenu)
	{
		BeginMainMenuState();
	}
	else if (NewState == RSActionGameInstanceState::MessageMenu)
	{
		BeginMessageMenuState();
	}
	else if (NewState == RSActionGameInstanceState::Playing)
	{
		BeginPlayingState();
	}
...
void URSActionGameInstance::BeginPendingInviteState()
{	
	if (LoadFrontEndMap(MainMenuMap))
	{				
		StartOnlinePrivilegeTask(IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate::CreateUObject(this, &URSActionGameInstance::OnUserCanPlayInvite), EUserPrivileges::CanPlayOnline, PendingInvite.UserId);
	}
	else
	{
		GotoState(RSActionGameInstanceState::WelcomeScreen);
...
void URSActionGameInstance::EndPendingInviteState()
{
	// cleanup in case the state changed before the pending invite was handled.
	CleanupOnlinePrivilegeTask();
}

void URSActionGameInstance::BeginWelcomeScreenState()
{
...
void URSActionGameInstance::EndWelcomeScreenState()
{
	if (WelcomeMenuUI.IsValid())
	{
...
void URSActionGameInstance::SetPresenceForLocalPlayers(const FString& StatusStr, const FVariantData& PresenceData)
{
...
void URSActionGameInstance::BeginMainMenuState()
{
...
void URSActionGameInstance::EndMainMenuState()
{
...
void URSActionGameInstance::BeginMessageMenuState()
{
	if (PendingMessage.DisplayString.IsEmpty())
	{
		UE_LOG(LogOnlineGame, Warning, TEXT("URSActionGameInstance::BeginMessageMenuState: Display string is empty"));
		GotoInitialState();
		return;
	}
...
void URSActionGameInstance::EndMessageMenuState()
{
...
void URSActionGameInstance::BeginPlayingState()
{
...
void URSActionGameInstance::EndPlayingState()
{
	// Disallow splitscreen
	GetGameViewportClient()->SetForceDisableSplitscreen( true );
...
void URSActionGameInstance::OnEndSessionComplete( FName SessionName, bool bWasSuccessful )
{
	UE_LOG(LogOnline, Log, TEXT("URSActionGameInstance::OnEndSessionComplete: Session=%s bWasSuccessful=%s"), *SessionName.ToString(), bWasSuccessful ? TEXT("true") : TEXT("false") );
...
void URSActionGameInstance::CleanupSessionOnReturnToMenu()
{
...
void URSActionGameInstance::LabelPlayerAsQuitter(ULocalPlayer* LocalPlayer) const
{
...
void URSActionGameInstance::RemoveNetworkFailureHandlers()
{
...
void URSActionGameInstance::AddNetworkFailureHandlers()
{
	// Add network/travel error handlers (if they are not already there)
	if (GEngine->OnTravelFailure().IsBoundToObject(this) == false)
	{
		TravelLocalSessionFailureDelegateHandle = GEngine->OnTravelFailure().AddUObject(this, &URSActionGameInstance::TravelLocalSessionFailure);
	}
}

TSubclassOf<UOnlineSession> URSActionGameInstance::GetOnlineSessionClass()
{
	return USoldierOnlineSessionClient::StaticClass();
}

bool URSActionGameInstance::HostQuickSession(ULocalPlayer& LocalPlayer, const FOnlineSessionSettings& SessionSettings)
{
	...
	if (ASoldierGameSession* const GameSession = GetGameSession())
	{
		...
		OnCreatePresenceSessionCompleteDelegateHandle = GameSession->OnCreatePresenceSessionComplete().AddUObject(this, &URSActionGameInstance::OnCreatePresenceSessionComplete);
...
bool URSActionGameInstance::HostGame(ULocalPlayer* LocalPlayer, const FString& GameType, const FString& InTravelURL)
{
...
bool URSActionGameInstance::JoinSession(ULocalPlayer* LocalPlayer, int32 SessionIndexInSearchResults)
{
	...
	if (GameSession)
	{
		...
		OnJoinSessionCompleteDelegateHandle = GameSession->OnJoinSessionComplete().AddUObject(this, &URSActionGameInstance::OnJoinSessionComplete);
...
bool URSActionGameInstance::JoinSession(ULocalPlayer* LocalPlayer, const FOnlineSessionSearchResult& SearchResult)
{
	// needs to tear anything down based on current state?
	ASoldierGameSession* const GameSession = GetGameSession();
	if (GameSession)
...
bool URSActionGameInstance::PlayDemo(ULocalPlayer* LocalPlayer, const FString& DemoName)
{
	ShowLoadingScreen();
...
void URSActionGameInstance::OnJoinSessionComplete(EOnJoinSessionCompleteResult::Type Result)
{
	// unhook the delegate
	ASoldierGameSession* const GameSession = GetGameSession();
	if (GameSession)
	...
	if (Result == EOnJoinSessionCompleteResult::Success && LocalPlayers.Num() > 1)
	{
		...
		if (Sessions.IsValid() && LocalPlayers[1]->GetPreferredUniqueNetId().IsValid())
		{
			Sessions->RegisterLocalPlayer(*LocalPlayers[1]->GetPreferredUniqueNetId(), NAME_GameSession,
				FOnRegisterLocalPlayerCompleteDelegate::CreateUObject(this, &URSActionGameInstance::OnRegisterJoiningLocalPlayerComplete));
		}
	}
	else
	{
...
void URSActionGameInstance::FinishJoinSession(EOnJoinSessionCompleteResult::Type Result)
{
...
void URSActionGameInstance::OnRegisterJoiningLocalPlayerComplete(const FUniqueNetId& PlayerId, EOnJoinSessionCompleteResult::Type Result)
{
	FinishJoinSession(Result);
}

void URSActionGameInstance::InternalTravelToSession(const FName& SessionName)
{
...
void URSActionGameInstance::OnCreatePresenceSessionComplete(FName SessionName, bool bWasSuccessful)
{
	ASoldierGameSession* const GameSession = GetGameSession();
	if (GameSession)
	{
		...
		if (bWasSuccessful && LocalPlayers.Num() > 1)
		{
			...
			{
				Sessions->RegisterLocalPlayer(*LocalPlayers[1]->GetPreferredUniqueNetId(), NAME_GameSession,
					FOnRegisterLocalPlayerCompleteDelegate::CreateUObject(this, &URSActionGameInstance::OnRegisterLocalPlayerComplete));
			}
		}
		else
		{
...
bool URSActionGameInstance::FindSessions(ULocalPlayer* PlayerOwner, bool bIsDedicatedServer, bool bFindLAN)
{
	bool bResult = false;
	...
	if (PlayerOwner)
	{
		...
		if (GameSession)
		{
			...
			OnSearchSessionsCompleteDelegateHandle = GameSession->OnFindSessionsComplete().AddUObject(this, &URSActionGameInstance::OnSearchSessionsComplete);
...
/** Callback which is intended to be called upon finding sessions */
void URSActionGameInstance::OnSearchSessionsComplete(bool bWasSuccessful)
{
...
bool URSActionGameInstance::Tick(float DeltaSeconds)
{
	...
	if (CurrentState != RSActionGameInstanceState::WelcomeScreen && SoldierViewport != nullptr)
	{
		// If at any point we aren't licensed (but we are after welcome screen) bounce them back to the welcome screen
		if (!bIsLicensed && CurrentState != RSActionGameInstanceState::None && SoldierViewport != nullptr && !SoldierViewport->IsShowingDialog())
		{
			...
			ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), RSActionGameInstanceState::WelcomeScreen );
		}
...
#endif
						FText::GetEmpty(),
						FOnClicked::CreateUObject(this, &URSActionGameInstance::OnControllerReconnectConfirm),
						FOnClicked()
					);
				}
			}
	...
	if (PendingInvite.UserId.IsValid() && PendingInvite.bPrivilegesCheckedAndAllowed && CurrentState == RSActionGameInstanceState::PendingInvite)
	{
		IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
		IOnlineSessionPtr Sessions = (OnlineSub != NULL) ? OnlineSub->GetSessionInterface() : NULL;
...
bool URSActionGameInstance::HandleOpenCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
...
bool URSActionGameInstance::HandleDisconnectCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
...
bool URSActionGameInstance::HandleTravelCommand(const TCHAR* Cmd, FOutputDevice& Ar, UWorld* InWorld)
{
...
void URSActionGameInstance::HandleSignInChangeMessaging()
{
...
void URSActionGameInstance::HandleUserLoginChanged(int32 GameUserIndex, ELoginStatus::Type PreviousLoginStatus, ELoginStatus::Type LoginStatus, const FUniqueNetId& UserId)
{
...
void URSActionGameInstance::HandleAppWillDeactivate()
{
	if (CurrentState == RSActionGameInstanceState::Playing)
	{
...
void URSActionGameInstance::HandleAppSuspend()
{
	...
	UE_LOG( LogOnline, Warning, TEXT( "URSActionGameInstance::HandleAppSuspend" ) );
	UWorld* const World = GetWorld(); 
	ASoldierGameState* const GameState = World != NULL ? World->GetGameState<ASoldierGameState>() : NULL;

	if ( CurrentState != RSActionGameInstanceState::None && CurrentState != GetInitialState() )
	{
		UE_LOG( LogOnline, Warning, TEXT( "URSActionGameInstance::HandleAppSuspend: Sending round end event for players" ) );
...
void URSActionGameInstance::HandleAppResume()
{
	UE_LOG( LogOnline, Log, TEXT( "URSActionGameInstance::HandleAppResume" ) );

	if ( CurrentState != RSActionGameInstanceState::None && CurrentState != GetInitialState() )
	{
		UE_LOG( LogOnline, Warning, TEXT( "URSActionGameInstance::HandleAppResume: Attempting to sign out players" ) );
		...
		for ( int32 i = 0; i < LocalPlayers.Num(); ++i )
		{
			...
			{
				UE_LOG( LogOnline, Log, TEXT( "URSActionGameInstance::HandleAppResume: Signed out during resume." ) );
				HandleSignInChangeMessaging();
				break;
			}
		}
...
void URSActionGameInstance::HandleAppLicenseUpdate()
{
	TSharedPtr<GenericApplication> GenericApplication = FSlateApplication::Get().GetPlatformApplication();
	bIsLicensed = GenericApplication->ApplicationLicenseValid();
}

void URSActionGameInstance::HandleSafeFrameChanged()
{
	UCanvas::UpdateAllCanvasSafeZoneData();
}

void URSActionGameInstance::RemoveExistingLocalPlayer(ULocalPlayer* ExistingPlayer)
{
...
void URSActionGameInstance::RemoveSplitScreenPlayers()
{
...
FReply URSActionGameInstance::OnPairingUsePreviousProfile()
{
...
FReply URSActionGameInstance::OnPairingUseNewProfile()
{
	HandleSignInChangeMessaging();
	return FReply::Handled();
}

void URSActionGameInstance::HandleControllerPairingChanged( int GameUserIndex, const FUniqueNetId& PreviousUser, const FUniqueNetId& NewUser )
{
	UE_LOG(LogOnlineGame, Log, TEXT("URSActionGameInstance::HandleControllerPairingChanged GameUserIndex %d PreviousUser '%s' NewUser '%s'"),
		GameUserIndex, *PreviousUser.ToString(), *NewUser.ToString());
	
	if ( CurrentState == RSActionGameInstanceState::WelcomeScreen )
	{
		// Don't care about pairing changes at welcome screen
		return;
	}

#if SOLDIER_CONSOLE_UI && PLATFORM_XBOXONE
	...
	if ( ControlledLocalPlayer != nullptr && ControlledLocalPlayer != NewLocalPlayer )
	{
		...
		if ( SoldierViewport != nullptr )
		{
			SoldierViewport->ShowDialog( 
				...
				FOnClicked::CreateUObject(this, &URSActionGameInstance::OnPairingUseNewProfile),
				FOnClicked::CreateUObject(this, &URSActionGameInstance::OnPairingUsePreviousProfile)
			);
		}
	}
#endif
...
void URSActionGameInstance::HandleControllerConnectionChange( bool bIsConnection, int32 Unused, int32 GameUserIndex )
{
	UE_LOG(LogOnlineGame, Log, TEXT("URSActionGameInstance::HandleControllerConnectionChange bIsConnection %d GameUserIndex %d"),
...
FReply URSActionGameInstance::OnControllerReconnectConfirm()
{
	USoldierGameViewportClient * SoldierViewport = Cast<USoldierGameViewportClient>(GetGameViewportClient());
	if(SoldierViewport)
	{
...
TSharedPtr< const FUniqueNetId > URSActionGameInstance::GetUniqueNetIdFromControllerId( const int ControllerId )
{
...
void URSActionGameInstance::SetOnlineMode(EOnlineMode InOnlineMode)
{
	OnlineMode = InOnlineMode;
	UpdateUsingMultiplayerFeatures(InOnlineMode == EOnlineMode::Online);
}

void URSActionGameInstance::UpdateUsingMultiplayerFeatures(bool bIsUsingMultiplayerFeatures)
{
	IOnlineSubsystem* OnlineSub = Online::GetSubsystem(GetWorld());
...
void URSActionGameInstance::TravelToSession(const FName& SessionName)
{
...
void URSActionGameInstance::SetIgnorePairingChangeForControllerId( const int32 ControllerId )
{
	IgnorePairingChangeForControllerId = ControllerId;
}

bool URSActionGameInstance::IsLocalPlayerOnline(ULocalPlayer* LocalPlayer)
{
	if (LocalPlayer == NULL)
	{
		return false;
...
bool URSActionGameInstance::IsLocalPlayerSignedIn(ULocalPlayer* LocalPlayer)
{
	if (LocalPlayer == NULL)
	{
		return false;
...
bool URSActionGameInstance::ValidatePlayerForOnlinePlay(ULocalPlayer* LocalPlayer)
{
	// Get the viewport
	USoldierGameViewportClient * SoldierViewport = Cast<USoldierGameViewportClient>(GetGameViewportClient());
	...
	if (CurrentConnectionStatus != EOnlineServerConnectionStatus::Connected)
	{
		...
		if (SoldierViewport != NULL)
		{
			...
			SoldierViewport->ShowDialog( 
				...
				OKButtonString,
				FText::GetEmpty(),
				FOnClicked::CreateUObject(this, &URSActionGameInstance::OnConfirmGeneric),
				FOnClicked::CreateUObject(this, &URSActionGameInstance::OnConfirmGeneric)
			);
		}
...
bool URSActionGameInstance::ValidatePlayerIsSignedIn(ULocalPlayer* LocalPlayer)
{
	// Get the viewport
	USoldierGameViewportClient * SoldierViewport = Cast<USoldierGameViewportClient>(GetGameViewportClient());
...
FReply URSActionGameInstance::OnConfirmGeneric()
{
	USoldierGameViewportClient * SoldierViewport = Cast<USoldierGameViewportClient>(GetGameViewportClient());
	if(SoldierViewport)
	{
...
void URSActionGameInstance::StartOnlinePrivilegeTask(const IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate& Delegate, EUserPrivileges::Type Privilege, TSharedPtr< const FUniqueNetId > UserId)
{
...
void URSActionGameInstance::CleanupOnlinePrivilegeTask()
{
...
void URSActionGameInstance::DisplayOnlinePrivilegeFailureDialogs(const FUniqueNetId& UserId, EUserPrivileges::Type Privilege, uint32 PrivilegeResults)
{	
...
void URSActionGameInstance::OnRegisterLocalPlayerComplete(const FUniqueNetId& PlayerId, EOnJoinSessionCompleteResult::Type Result)
{
	FinishSessionCreation(Result);
}

void URSActionGameInstance::FinishSessionCreation(EOnJoinSessionCompleteResult::Type Result)
{
	if (Result == EOnJoinSessionCompleteResult::Success)
	{
...
FString URSActionGameInstance::GetQuickMatchUrl()
{
	static const FString QuickMatchUrl(TEXT("/Game/Maps/Highrise?game=TDM?listen"));
	return QuickMatchUrl;
}

void URSActionGameInstance::BeginHostingQuickMatch()
{
	ShowLoadingScreen();
	GotoState(RSActionGameInstanceState::Playing);
...
void URSActionGameInstance::OnPlayTogetherEventReceived(const int32 UserIndex, const TArray<TSharedPtr<const FUniqueNetId>>& UserIdList)
{
	...
	{
		StartOnlinePrivilegeTask(IOnlineIdentity::FOnGetUserPrivilegeCompleteDelegate::CreateUObject(this, &URSActionGameInstance::OnUserCanPlayTogether), EUserPrivileges::CanPlayOnline, PendingInvite.UserId);
	}
	else
	{
		GotoState(RSActionGameInstanceState::MainMenu);
...
void URSActionGameInstance::SendPlayTogetherInvites()
{
	const IOnlineSubsystem* const OnlineSub = Online::GetSubsystem(GetWorld());
	check(OnlineSub);
...
void URSActionGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken, const FOnEncryptionKeyResponse& Delegate)
{
...
void URSActionGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
