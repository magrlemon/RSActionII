...
struct FSoldierGameGlobalDelegateInit
{
	FSoldierGameGlobalDelegateInit()
	{
		FPakPlatformFile::GetPakChunkSignatureCheckFailedHandler().AddStatic(FSoldierGameGlobalDelegateInit::HandlePakChunkSignatureCheckFailed);
		FPakPlatformFile::GetPakMasterSignatureTableCheckFailureHandler().AddStatic(FSoldierGameGlobalDelegateInit::HandlePakMasterSignatureTableCheckFailure);
	}

	static void HandlePakChunkSignatureCheckFailed(const FPakChunkSignatureCheckFailedData& Data)
	{
		UE_LOG(LogSoldier, Fatal, TEXT("Pak chunk signature check failed!"));
	}

	static void HandlePakMasterSignatureTableCheckFailure(const FString& InPakFilename)
	{
		UE_LOG(LogSoldier, Fatal, TEXT("Pak master signature table check failed for pak '%s'"), *InPakFilename);
	}
}
GSoldierGameGlobalDelegateInit;
	...
	if (URL == TEXT("/index.html?scoreboard"))
	{
		...
		if (GameEngine)
		{
			...
			{			
				...
				{
					// get the shoter game
					ASoldierGameState* const GameState = Player->PlayerController->GetWorld()->GetGameState<ASoldierGameState>();
...
void InitializeSoldierGameDelegates()
{
