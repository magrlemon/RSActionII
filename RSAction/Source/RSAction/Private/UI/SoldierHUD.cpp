...
ASoldierHUD::ASoldierHUD(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	HitNotifyIcon[ESoldierHudPosition::Left] = UCanvas::MakeIcon(HitNotifyTexture,  158, 831, 585, 392);	
	HitNotifyIcon[ESoldierHudPosition::FrontLeft] = UCanvas::MakeIcon(HitNotifyTexture, 369, 434, 460, 378);	
	HitNotifyIcon[ESoldierHudPosition::Front] = UCanvas::MakeIcon(HitNotifyTexture,  848, 284, 361, 395);	
	HitNotifyIcon[ESoldierHudPosition::FrontRight] = UCanvas::MakeIcon(HitNotifyTexture,  1212, 397, 427, 394);	
	HitNotifyIcon[ESoldierHudPosition::Right] = UCanvas::MakeIcon(HitNotifyTexture, 1350, 844, 547, 321);	
	HitNotifyIcon[ESoldierHudPosition::BackRight] = UCanvas::MakeIcon(HitNotifyTexture, 1232, 1241, 458, 341);	
	HitNotifyIcon[ESoldierHudPosition::Back] = UCanvas::MakeIcon(HitNotifyTexture,  862, 1384, 353, 408);	
	HitNotifyIcon[ESoldierHudPosition::BackLeft] = UCanvas::MakeIcon(HitNotifyTexture, 454, 1251, 371, 410);	
	...
	Crosshair[ESoldierCrosshairDirection::Left] = UCanvas::MakeIcon(HUDMainTexture, 43, 402, 25, 9); // left
	Crosshair[ESoldierCrosshairDirection::Right] = UCanvas::MakeIcon(HUDMainTexture, 88, 402, 25, 9); // right
	Crosshair[ESoldierCrosshairDirection::Top] = UCanvas::MakeIcon(HUDMainTexture, 74, 371, 9, 25); // top
	Crosshair[ESoldierCrosshairDirection::Bottom] = UCanvas::MakeIcon(HUDMainTexture, 74, 415, 9, 25); // bottom
	Crosshair[ESoldierCrosshairDirection::Center] = UCanvas::MakeIcon(HUDMainTexture, 75, 403, 7, 7); // center

	Offsets[ESoldierHudPosition::Left] = FVector2D(173,0);	
	Offsets[ESoldierHudPosition::FrontLeft] = FVector2D(120,125);	
	Offsets[ESoldierHudPosition::Front] = FVector2D(0,173);	
	Offsets[ESoldierHudPosition::FrontRight] = FVector2D(-120,125);
	Offsets[ESoldierHudPosition::Right] = FVector2D(-173,0);	
	Offsets[ESoldierHudPosition::BackRight] = FVector2D(-120,-125);
	Offsets[ESoldierHudPosition::Back] = FVector2D(0,-173);
	Offsets[ESoldierHudPosition::BackLeft] = FVector2D(120,-125);
...
void ASoldierHUD::SetMatchState(ESoldierMatchState::Type NewState)
{
	MatchState = NewState;
}

ESoldierMatchState::Type ASoldierHUD::GetMatchState() const
{
	return MatchState;
}
...
void ASoldierHUD::DrawMatchTimerAndPosition()
{
	...
	if (MyGameState && MatchState == ESoldierMatchState::Playing)
	{
	...
	if (MyGameState && MyGameState->RemainingTime > 0)
	{
		...
		if (MyPC && MyGameState && MatchState == ESoldierMatchState::Playing)
		{
	...
	if (PCOwner)
	{
			...
			&& (Pawn->IsTargeting() || (!Pawn->IsTargeting() && !Pawn->GetWeapon()->bHideCrosshairWhileNotAiming)))
		{
			...
			if (Pawn->IsTargeting() && MyWeapon && MyWeapon->UseLaserDot)
			{
				...
				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Center],
					CenterX - (*CurrentCrosshair[ESoldierCrosshairDirection::Center]).UL*ScaleUI / 2.0f,
					CenterY - (*CurrentCrosshair[ESoldierCrosshairDirection::Center]).VL*ScaleUI / 2.0f, ScaleUI);
			}
			else
			{
				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Center], 
				...
				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Left],
					CenterX - 1 - (*CurrentCrosshair[ESoldierCrosshairDirection::Left]).UL * ScaleUI - CrossSpread * ScaleUI, 
					CenterY - (*CurrentCrosshair[ESoldierCrosshairDirection::Left]).VL*ScaleUI / 2.0f, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Right], 
					CenterX + CrossSpread * ScaleUI, 
					CenterY - (*CurrentCrosshair[ESoldierCrosshairDirection::Right]).VL * ScaleUI / 2.0f, ScaleUI);

				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Top], 
					CenterX - (*CurrentCrosshair[ESoldierCrosshairDirection::Top]).UL * ScaleUI / 2.0f,
					CenterY - 1 - (*CurrentCrosshair[ESoldierCrosshairDirection::Top]).VL * ScaleUI - CrossSpread * ScaleUI, ScaleUI);
				Canvas->DrawIcon(*CurrentCrosshair[ESoldierCrosshairDirection::Bottom],
					CenterX - (*CurrentCrosshair[ESoldierCrosshairDirection::Bottom]).UL * ScaleUI / 2.0f,
...
bool ASoldierHUD::ShowScoreboard(bool bEnable, bool bFocus)
{
	...
	if (bIsScoreBoardVisible)
	{
		...
		.Padding(FMargin(50))
		[
			SAssignNew(ScoreboardWidget, SSoldierScoreboardWidget)
...
void ASoldierHUD::ToggleChat()
{
	...
	if( (SoldierPC == NULL ) || SoldierPC->IsGameMenuVisible() || GetMatchState() == ESoldierMatchState::Warmup )
	{
		return;
	}
...
{
	return GetMatchState() == ESoldierMatchState::Lost || GetMatchState() == ESoldierMatchState::Won;
}
