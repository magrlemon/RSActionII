...
//class declare
class SSoldierScoreboardWidget : public SBorder
{

	SLATE_BEGIN_ARGS(SSoldierScoreboardWidget)
	{}

	SLATE_ARGUMENT(TWeakObjectPtr<APlayerController>, PCOwner)

	SLATE_ATTRIBUTE(ESoldierMatchState::Type, MatchState)
...
protected:
	...
	ASoldierPlayerState* GetSortedPlayerState(const FTeamPlayer& TeamPlayer) const;
