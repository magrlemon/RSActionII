...
{
public:
	SLATE_BEGIN_ARGS(SSoldierReplayTimeline)
		: _DemoDriver(nullptr)
...
void SSoldierReplayTimeline::Construct(const FArguments& InArgs)
{
	...
	[
		...
		.OnMouseButtonDown(this, &SSoldierReplayTimeline::OnTimelineClicked)
	];
}

FVector2D SSoldierReplayTimeline::ComputeDesiredSize(float LayoutScaleMultiplier) const
{
...
int32 SSoldierReplayTimeline::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
...
FReply SSoldierReplayTimeline::OnTimelineClicked(const FGeometry& Geometry, const FPointerEvent& Event)
{
...
void SSoldierDemoHUD::Construct(const FArguments& InArgs)
{	
	...
	[
		...
		[		
			...
			[
				SNew(STextBlock)
				.Margin(3.0f)
				.Text(this, &SSoldierDemoHUD::GetCurrentReplayTime)
			]
			...
			[
				...
				+SOverlay::Slot()
				[
					SNew(SSoldierReplayTimeline)
					.DemoDriver(PlayerOwner->GetWorld()->DemoNetDriver)
					.BackgroundBrush(FSoldierStyle::Get().GetBrush("SoldierGame.ReplayTimelineBorder"))
					.BackgroundPadding(FMargin(0.0f, 3.0))
					.IndicatorBrush(FSoldierStyle::Get().GetBrush("SoldierGame.ReplayTimelineIndicator"))
				]
				...
				[
					...
					.Margin(3.0f)
					.Text(this, &SSoldierDemoHUD::GetPlaybackSpeed)
				]
			]
			...
			[
				SNew(STextBlock)
				.Margin(3.0f)
				.Text(this, &SSoldierDemoHUD::GetTotalReplayTime)
			]
		]
		...
		[
			...
			[
				...
				.IsChecked(this, &SSoldierDemoHUD::IsPauseChecked)
				.OnCheckStateChanged(this, &SSoldierDemoHUD::OnPauseCheckStateChanged)
				[
					SNew(SImage)
					.Image(FSoldierStyle::Get().GetBrush("SoldierGame.ReplayPauseIcon"))
				]
			]
...
FText SSoldierDemoHUD::GetCurrentReplayTime() const
{
	...
	UDemoNetDriver* DemoDriver = PlayerOwner->GetWorld()->DemoNetDriver;
...
FText SSoldierDemoHUD::GetTotalReplayTime() const
{
	if (!PlayerOwner.IsValid())
	{
		return FText::GetEmpty();
...
FText SSoldierDemoHUD::GetPlaybackSpeed() const
{
...
void SSoldierDemoHUD::OnPauseCheckStateChanged(ECheckBoxState CheckState) const
{
	if (!PlayerOwner.IsValid())
	{
		return;
