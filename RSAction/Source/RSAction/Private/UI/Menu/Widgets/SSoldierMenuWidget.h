...
//class declare
class SSoldierMenuWidget : public SCompoundWidget
{
public:
	SLATE_BEGIN_ARGS(SSoldierMenuWidget)
	: _PlayerOwner()
	, _IsGameMenu(false)
	{
	}
	...
	void UpdateArrows(TSharedPtr<FSoldierMenuItem> MenuItem);
	...
	TArray<FSoldierMenuInfo> MenuHistory;
...
private:
	...
	FText GetOptionText(TSharedPtr<FSoldierMenuItem> MenuItem) const;
	...
	const struct FSoldierMenuStyle *MenuStyle;
};

namespace MenuHelper
{
	FORCEINLINE void EnsureValid(TSharedPtr<FSoldierMenuItem>& MenuItem)
	{
		if (!MenuItem.IsValid())
		{
			MenuItem = FSoldierMenuItem::CreateRoot();
		}
	}

	//Helper functions for creating menu items
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuItem(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuItem(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FSoldierMenuItem::FOnConfirmMenuItem::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuItemSP(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text, UserClass* inObj, typename FSoldierMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text));
	...
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuOption(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text, OptionsList));
		MenuItem->SubMenu.Add(Item);
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuOption(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FSoldierMenuItem::FOnOptionChanged::TUObjectMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text, OptionsList));
	...
	template< class UserClass >	
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddMenuOptionSP(TSharedPtr<FSoldierMenuItem>& MenuItem, const FText& Text, const TArray<FText>& OptionsList, UserClass* inObj, typename FSoldierMenuItem::FOnOptionChanged::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
		EnsureValid(MenuItem);
		TSharedPtr<FSoldierMenuItem> Item = MakeShareable(new FSoldierMenuItem(Text, OptionsList));
	...
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddExistingMenuItem(TSharedPtr<FSoldierMenuItem>& MenuItem, TSharedRef<FSoldierMenuItem> SubMenuItem)
	{
		EnsureValid(MenuItem);
	...
	FORCEINLINE TSharedRef<FSoldierMenuItem> AddCustomMenuItem(TSharedPtr<FSoldierMenuItem>& MenuItem, TSharedPtr<SWidget> CustomWidget)
	{
		EnsureValid(MenuItem);
		MenuItem->SubMenu.Add(MakeShareable(new FSoldierMenuItem(CustomWidget)));
		return MenuItem->SubMenu.Last().ToSharedRef();
	}

	FORCEINLINE void ClearSubMenu(TSharedPtr<FSoldierMenuItem>& MenuItem)
	{
		EnsureValid(MenuItem);
	...
	template< class UserClass >	
	FORCEINLINE void PlaySoundAndCall(UWorld* World, const FSlateSound& Sound, int32 UserIndex, UserClass* inObj, typename FSoldierMenuItem::FOnConfirmMenuItem::TSPMethodDelegate< UserClass >::FMethodPtr inMethod)
	{
