...
void SSoldierMenuItem::Construct(const FArguments& InArgs)
{
	ItemStyle = &FSoldierStyle::Get().GetWidgetStyle<FSoldierMenuItemStyle>("DefaultSoldierMenuItemStyle");
	...
	[
		...
		[
			...
			[
				SNew(SImage)
				.ColorAndOpacity(this,&SSoldierMenuItem::GetButtonBgColor)
		...
		[
			...
			.TextStyle(FSoldierStyle::Get(), "SoldierGame.MenuTextStyle")
			.ColorAndOpacity(this,&SSoldierMenuItem::GetButtonTextColor)
			.ShadowColorAndOpacity(this, &SSoldierMenuItem::GetButtonTextShadowColor)
			.Text(Text)
		]
		+SOverlay::Slot()
		.HAlign(HAlign_Right)
		...
		[
			...
			[
				...
				.Visibility(this,&SSoldierMenuItem::GetLeftArrowVisibility)
				.OnMouseButtonDown(this,&SSoldierMenuItem::OnLeftArrowDown)
				[
					SNew(SOverlay)
					+SOverlay::Slot()
			...
			+SHorizontalBox::Slot()
			.AutoWidth()
			.Padding(TAttribute<FMargin>(this, &SSoldierMenuItem::GetOptionPadding))
			[
				SNew(STextBlock)
				.TextStyle(FSoldierStyle::Get(), "SoldierGame.MenuTextStyle")
			...
			[
				...
				.Visibility(this,&SSoldierMenuItem::GetRightArrowVisibility)
				.OnMouseButtonDown(this,&SSoldierMenuItem::OnRightArrowDown)
				[
					SNew(SOverlay)
					+SOverlay::Slot()
					.HAlign(HAlign_Center)
...
FReply SSoldierMenuItem::OnRightArrowDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SSoldierMenuItem::OnLeftArrowDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	FReply Result = FReply::Unhandled();
...
EVisibility SSoldierMenuItem::GetLeftArrowVisibility() const
{
	return LeftArrowVisible;
}

EVisibility SSoldierMenuItem::GetRightArrowVisibility() const
{
	return RightArrowVisible;
}

FMargin SSoldierMenuItem::GetOptionPadding() const
{
	return RightArrowVisible == EVisibility::Visible ? FMargin(0) : FMargin(0,0,ItemMargin,0);
}

FSlateColor SSoldierMenuItem::GetButtonTextColor() const
{
	FLinearColor Result;
...
FLinearColor SSoldierMenuItem::GetButtonTextShadowColor() const
{
	FLinearColor Result;
	if (bIsActiveMenuItem)
	{
...
FSlateColor SSoldierMenuItem::GetButtonBgColor() const
{
...
FReply SSoldierMenuItem::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SSoldierMenuItem::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	return FReply::Handled();
}

void SSoldierMenuItem::SetMenuItemActive(bool bIsMenuItemActive)
{
	this->bIsActiveMenuItem = bIsMenuItemActive;
}

void SSoldierMenuItem::UpdateItemText(const FText& UpdatedText)
{
	Text = UpdatedText;
