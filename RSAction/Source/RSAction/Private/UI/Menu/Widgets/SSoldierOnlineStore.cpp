	...
	[
		...
		[
			...
			[
				...
				.OnGenerateRow(this, &SSoldierOnlineStore::MakeListViewWidget)
				.OnSelectionChanged(this, &SSoldierOnlineStore::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&SSoldierOnlineStore::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(SRichTextBlock)
				.Text(this, &SSoldierOnlineStore::GetBottomText)
...
FText SSoldierOnlineStore::GetBottomText() const
{
	return StatusText;
}
 ...
 */
void SSoldierOnlineStore::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	SCompoundWidget::Tick( AllottedGeometry, InCurrentTime, InDeltaTime );
}

/** Returns logged in user */
TSharedPtr<const FUniqueNetId> SSoldierOnlineStore::GetLoggedInUser()
{
...
void SSoldierOnlineStore::BeginGettingOffers()
{
...
void SSoldierOnlineStore::SetStoreState(EStoreState NewState)
{
...
void SSoldierOnlineStore::PurchaseOffer()
{
	if (State != EStoreState::Browsing)
	{
...
void SSoldierOnlineStore::MarkAsPurchased(const TArray<FUniqueOfferId> & Offers)
{
...
void SSoldierOnlineStore::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SSoldierOnlineStore::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SSoldierOnlineStore::EntrySelectionChanged(TSharedPtr<FStoreEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void SSoldierOnlineStore::OnListItemDoubleClicked(TSharedPtr<FStoreEntry> InItem)
{
	SelectedItem = InItem;
	PurchaseOffer();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void SSoldierOnlineStore::MoveSelection(int32 MoveBy)
{
...
FReply SSoldierOnlineStore::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent) 
{
...
TSharedRef<ITableRow> SSoldierOnlineStore::MakeListViewWidget(TSharedPtr<FStoreEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
	...
	public:
		...
		TSharedRef<SWidget> GenerateWidgetForColumn(const FName& ColumnName)
		{
			...
			return SNew(STextBlock)
				.Text(ItemText)
				.TextStyle(FSoldierStyle::Get(), "SoldierGame.MenuStoreListTextStyle");
		}
