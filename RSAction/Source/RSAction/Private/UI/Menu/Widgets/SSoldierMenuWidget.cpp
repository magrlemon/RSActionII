...
void SSoldierMenuWidget::Construct(const FArguments& InArgs)
{
	MenuStyle = &FSoldierStyle::Get().GetWidgetStyle<FSoldierMenuStyle>("DefaultSoldierMenuStyle");
	...
	Visibility.Bind(this, &SSoldierMenuWidget::GetSlateVisibility);
	...
	const FText ProfileSwap = SoldierUIHelpers::Get().GetProfileSwapText();
	const TSharedRef< FSlateFontMeasure > FontMeasure = FSlateApplication::Get().GetRenderer()->GetFontMeasureService();
	const FSlateFontInfo PlayerNameFontInfo = FSoldierStyle::Get().GetWidgetStyle<FTextBlockStyle>("SoldierGame.MenuProfileNameStyle").Font;
	const FSlateFontInfo ProfileSwapFontInfo = FSoldierStyle::Get().GetWidgetStyle<FTextBlockStyle>("SoldierGame.MenuServerListTextStyle").Font;
	...
	[
		...
		[
			...
			[
				...
				[
					SNew(SImage)
					.Visibility(this, &SSoldierMenuWidget::GetProfileSwapVisibility)
					.ColorAndOpacity(this, &SSoldierMenuWidget::GetHeaderColor)
			...
			[
				...
				[
					SNew(STextBlock)
					.TextStyle(FSoldierStyle::Get(), "SoldierGame.MenuProfileNameStyle")
		...
		[
			...
			.HAlign(HAlign_Left)
			.VAlign(VAlign_Top)
			.Padding(TAttribute<FMargin>(this,&SSoldierMenuWidget::GetMenuOffset))
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot()
				.AutoHeight()
				[
					...
					[
						...
						[
							...
							.ColorAndOpacity(MenuTitleTextColor)
							.Text(this,&SSoldierMenuWidget::GetMenuTitle)
						]
					]
				]
				+ SVerticalBox::Slot()
				...
				[
					...
					.ColorAndOpacity(this, &SSoldierMenuWidget::GetBottomColor)
					.VAlign(VAlign_Top)
					.HAlign(HAlign_Left)
					[
						SNew(SHorizontalBox)
						...
						[
							...
							+ SVerticalBox::Slot()
							.AutoHeight()
							.Padding(TAttribute<FMargin>(this,&SSoldierMenuWidget::GetLeftMenuOffset))
							[
								SNew(SBorder)
								.BorderImage(&MenuStyle->LeftBackgroundBrush)
								.BorderBackgroundColor(FLinearColor(1.0f, 1.0f, 1.0f, 1.0f))
								.Padding(FMargin(OutlineWidth))
								.DesiredSizeScale(this, &SSoldierMenuWidget::GetBottomScale)
								.VAlign(VAlign_Top)
								.HAlign(HAlign_Left)
								[
						...
						[
							...
							+ SVerticalBox::Slot()
							.Padding(TAttribute<FMargin>(this,&SSoldierMenuWidget::GetSubMenuOffset))
							.AutoHeight()
							[
								SNew(SBorder)
...
EVisibility SSoldierMenuWidget::GetSlateVisibility() const
{
	return bConsoleVisible ? EVisibility::HitTestInvisible : EVisibility::Visible;
}

FText SSoldierMenuWidget::GetMenuTitle() const 
{
	return CurrentMenuTitle;
}

FMargin SSoldierMenuWidget::GetProfileSwapOffset() const
{
	return FMargin(0.0f, 50.0f, 50.0f, 0.0f);
}

bool SSoldierMenuWidget::IsProfileSwapActive() const
{
#if PROFILE_SWAPPING
...
EVisibility SSoldierMenuWidget::GetProfileSwapVisibility() const
{
	return IsProfileSwapActive() ? EVisibility::Visible : EVisibility::Collapsed;
}

UWorld* SSoldierMenuWidget::GetWorld() const
{
	return PlayerOwner.IsValid() ? PlayerOwner->GetWorld() : nullptr;
}

bool SSoldierMenuWidget::ProfileUISwap(const int ControllerIndex) const
{
...
void SSoldierMenuWidget::HandleProfileUISwapClosed(TSharedPtr<const FUniqueNetId> UniqueId, const int ControllerIndex, const FOnlineError& Error)
{
	...
	USoldierLocalPlayer* LocalPlayer = Cast<USoldierLocalPlayer>(PlayerOwner.Get());
	LocalPlayer->LoadPersistentUser();
}

void SSoldierMenuWidget::LockControls(bool bEnable)
{
	bControlsLocked = bEnable;
}

int32 SSoldierMenuWidget::GetOwnerUserIndex()
{
	return PlayerOwner.IsValid() ? PlayerOwner->GetControllerId() : 0;
}

int32 SSoldierMenuWidget::GetMenuLevel()
{
...
void SSoldierMenuWidget::HideMenu()
{
	if (!bMenuHiding)
	{
...
void SSoldierMenuWidget::SetupAnimations()
{
	//Setup a curve
...
void SSoldierMenuWidget::BuildLeftPanel(bool bInGoingBack)
{
	...
	if (bLeftMenuChanging)
	{
		...
		{
			FSoldierMenuInfo MenuInfo = MenuHistory.Pop();
		...
		{
			MenuHistory.Push(FSoldierMenuInfo(CurrentMenu, SelectedIndex, CurrentMenuTitle));
	...
	for(int32 i = 0; i < CurrentMenu.Num(); ++i)
	{
		if (CurrentMenu[i]->bVisible)
		{
			...
			if (CurrentMenu[i]->MenuItemType == ESoldierMenuItemType::Standard)
			{
				TmpWidget = SAssignNew(CurrentMenu[i]->Widget, SSoldierMenuItem)
					.PlayerOwner(PlayerOwner)
					.OnClicked(this, &SSoldierMenuWidget::ButtonClicked, i)
					.Text(CurrentMenu[i]->GetText())
					.bIsMultichoice(false);
			} 
			else if (CurrentMenu[i]->MenuItemType == ESoldierMenuItemType::MultiChoice)
			{
				TmpWidget = SAssignNew(CurrentMenu[i]->Widget, SSoldierMenuItem)
					.PlayerOwner(PlayerOwner)
					.OnClicked(this, &SSoldierMenuWidget::ButtonClicked, i)
					.Text(CurrentMenu[i]->GetText() )
					.bIsMultichoice(true)
					.OnArrowPressed(this, &SSoldierMenuWidget::ChangeOption)
					.OptionText(this, &SSoldierMenuWidget::GetOptionText, CurrentMenu[i]);
				UpdateArrows(CurrentMenu[i]);
			}
			else if (CurrentMenu[i]->MenuItemType == ESoldierMenuItemType::CustomWidget)
			{
	...
	TSharedPtr<FSoldierMenuItem> FirstMenuItem = CurrentMenu.IsValidIndex(SelectedIndex) ? CurrentMenu[SelectedIndex] : NULL;
	if (FirstMenuItem.IsValid() && FirstMenuItem->MenuItemType != ESoldierMenuItemType::CustomWidget)
	{
...
FText SSoldierMenuWidget::GetOptionText(TSharedPtr<FSoldierMenuItem> MenuItem) const
{
...
void SSoldierMenuWidget::BuildRightPanel()
{
	...
	for(int32 i = 0; i < NextMenu.Num(); ++i)
	{
		...
		{
			TSharedPtr<SSoldierMenuItem> TmpButton;
			//Custom menu items are not supported in the right panel
			if (NextMenu[i]->MenuItemType == ESoldierMenuItemType::Standard)
			{
				TmpButton = SAssignNew(NextMenu[i]->Widget, SSoldierMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText())
					.InactiveTextAlpha(0.3f)
					.bIsMultichoice(false);
			}
			else if (NextMenu[i]->MenuItemType == ESoldierMenuItemType::MultiChoice)
			{
				TmpButton = SAssignNew(NextMenu[i]->Widget, SSoldierMenuItem)
					.PlayerOwner(PlayerOwner)
					.Text(NextMenu[i]->GetText() )
					.InactiveTextAlpha(0.3f)
					.bIsMultichoice(true)
					.OptionText(this, &SSoldierMenuWidget::GetOptionText, NextMenu[i]);
			}
...
void SSoldierMenuWidget::UpdateArrows(TSharedPtr<FSoldierMenuItem> MenuItem)
{
...
void SSoldierMenuWidget::EnterSubMenu()
{
...
void SSoldierMenuWidget::MenuGoBack(bool bSilent)
{
...
void SSoldierMenuWidget::ConfirmMenuItem()
{
...
void SSoldierMenuWidget::ControllerFacebuttonLeftPressed()
{
...
void SSoldierMenuWidget::ControllerUpInputPressed()
{
...
void SSoldierMenuWidget::ControllerDownInputPressed()
{
...
void SSoldierMenuWidget::ControllerFacebuttonDownPressed()
{
...
void SSoldierMenuWidget::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
	...
	if (MenuWidgetAnimation.IsAtEnd())
	{
		if (bLeftMenuChanging)
		{
			...
			if (!LeftMenuWidgetAnimation.IsPlaying())
			{
				...
				if (CurrentMenu.Num() == 1 && CurrentMenu.Top()->MenuItemType == ESoldierMenuItemType::CustomWidget)
				{
...
FMargin SSoldierMenuWidget::GetMenuOffset() const
{
...
FMargin SSoldierMenuWidget::GetLeftMenuOffset() const
{
...
FMargin SSoldierMenuWidget::GetSubMenuOffset() const
{
...
FReply SSoldierMenuWidget::ButtonClicked(int32 ButtonIndex)
{
	...
	{
		TSharedPtr<SSoldierMenuItem> MenuItem = CurrentMenu[SelectedIndex]->Widget;
...
void SSoldierMenuWidget::FadeIn()
{
...
FReply SSoldierMenuWidget::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
void SSoldierMenuWidget::ChangeOption(int32 MoveBy)
{
	TSharedPtr<FSoldierMenuItem> MenuItem = CurrentMenu[SelectedIndex];

	const int32 MinIndex = MenuItem->MinMultiChoiceIndex > -1 ? MenuItem->MinMultiChoiceIndex : 0;
	const int32 MaxIndex = MenuItem->MaxMultiChoiceIndex > -1 ? MenuItem->MaxMultiChoiceIndex : MenuItem->MultiChoice.Num()-1;
	const int32 CurIndex = MenuItem->SelectedMultiChoice;

	if (MenuItem->MenuItemType == ESoldierMenuItemType::MultiChoice)
	{
...
int32 SSoldierMenuWidget::GetNextValidIndex(int32 MoveBy)
{
...
FReply SSoldierMenuWidget::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent)
{
...
FReply SSoldierMenuWidget::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
	//Focus the custom widget
	if (CurrentMenu.Num() == 1 && CurrentMenu.Top()->MenuItemType == ESoldierMenuItemType::CustomWidget)
	{
