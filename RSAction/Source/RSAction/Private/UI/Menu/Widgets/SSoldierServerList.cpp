	...
	[
		...
		[
			...
			[
				...
				.OnGenerateRow(this, &SSoldierServerList::MakeListViewWidget)
				.OnSelectionChanged(this, &SSoldierServerList::EntrySelectionChanged)
				.OnMouseButtonDoubleClick(this,&SSoldierServerList::OnListItemDoubleClicked)
				.HeaderRow(
					SNew(SHeaderRow)
		...
		[
			...
			[
				SNew(SRichTextBlock)
				.Text(this, &SSoldierServerList::GetBottomText)
...
void SSoldierServerList::UpdateSearchStatus()
{
	...
	if (SoldierSession)
	{
		...
		UE_LOG(LogOnlineGame, Log, TEXT("SoldierSession->GetSearchResultStatus: %s"), EOnlineAsyncTaskState::ToString(SearchState) );
...
FText SSoldierServerList::GetBottomText() const
{
	 return StatusText;
}
 ...
 */
void SSoldierServerList::Tick( const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime )
{
...
void SSoldierServerList::BeginServerSearch(bool bLANMatch, bool bIsDedicatedServer, const FString& InMapFilterName)
{
...
void SSoldierServerList::OnServerSearchFinished()
{
	bSearchingForServers = false;
...
void SSoldierServerList::UpdateServerList()
{
...
void SSoldierServerList::ConnectToServer()
{
	if (bSearchingForServers)
	{
		// unsafe
...
void SSoldierServerList::OnFocusLost( const FFocusEvent& InFocusEvent )
{
...
FReply SSoldierServerList::OnFocusReceived(const FGeometry& MyGeometry, const FFocusEvent& InFocusEvent)
{
...
void SSoldierServerList::EntrySelectionChanged(TSharedPtr<FServerEntry> InItem, ESelectInfo::Type SelectInfo)
{
	SelectedItem = InItem;
}

void SSoldierServerList::OnListItemDoubleClicked(TSharedPtr<FServerEntry> InItem)
{
	SelectedItem = InItem;
	ConnectToServer();
	FSlateApplication::Get().SetKeyboardFocus(SharedThis(this));
...
void SSoldierServerList::MoveSelection(int32 MoveBy)
{
	int32 SelectedItemIndex = ServerList.IndexOfByKey(SelectedItem);
...
FReply SSoldierServerList::OnKeyDown(const FGeometry& MyGeometry, const FKeyEvent& InKeyEvent) 
{
...
TSharedRef<ITableRow> SSoldierServerList::MakeListViewWidget(TSharedPtr<FServerEntry> Item, const TSharedRef<STableViewBase>& OwnerTable)
{
