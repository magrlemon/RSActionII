...
FText SoldierUIHelpers::GetProfileOpenText() const
{
...
bool SoldierUIHelpers::ProfileOpenedUI(UWorld* World, const FUniqueNetId& Requestor, const FUniqueNetId& Requestee, const FOnProfileUIClosedDelegate* Delegate) const
{
...
FText SoldierUIHelpers::GetProfileSwapText() const
{
	// @todo: replace button with icon
//#if PLATFORM_XBOXONE
...
bool SoldierUIHelpers::ProfileSwapUI(UWorld* World, const int ControllerIndex, bool bShowOnlineOnly, const FOnLoginUIClosedDelegate* Delegate) const
{
	const IOnlineSubsystem* OnlineSub = Online::GetSubsystem(World);
	if (OnlineSub)
	{
