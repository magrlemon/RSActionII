...
void ASoldierGameMode::InitGame(const FString& MapName, const FString& Options, FString& ErrorMessage)
{
	...
	if (GameInstance && Cast<URSActionGameInstance>(GameInstance)->GetOnlineMode() != EOnlineMode::Offline)
	{
		bPauseable = false;
	}
}
...
void ASoldierGameMode::RequestFinishAndExitToMainMenu()
{
	...
	URSActionGameInstance* const GameInstance = Cast<URSActionGameInstance>(GetGameInstance());
	if (GameInstance)
	{
