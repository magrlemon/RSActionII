		...
		//if (World)
		{
			...
			if (NetDriver)
			{
				switch (FailureType)
				{
					...
					{
						URSActionGameInstance* const SoldierInstance = Cast<URSActionGameInstance>(GameInstance);
						if (SoldierInstance && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// this is the case
							SoldierInstance->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), RSActionGameInstanceState::MainMenu, false );
						}
						break;
					}
					case ENetworkFailure::PendingConnectionFailure:						
					{
						URSActionGameInstance* const GI = Cast<URSActionGameInstance>(GameInstance);
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( FText::FromString(ErrorString), OKButton, FText::GetEmpty(), RSActionGameInstanceState::MainMenu, false );
						}
						break;
					}
					...
					case ENetworkFailure::ConnectionTimeout:
					{
						...
						if (GI && NetDriver->GetNetMode() == NM_Client)
						{
							...
							// We're going to make the assumption that someone else has a better message than "Lost connection to host" if
							// this is the case
							GI->ShowMessageThenGotoState( ReturnReason, OKButton, FText::GetEmpty(), RSActionGameInstanceState::MainMenu, false );
						}
						break;
					}
