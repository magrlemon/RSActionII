...
struct FDeathMessage
{
	...
	TWeakObjectPtr<class USoldierDamageType> DamageType;
...
public:
	...
	/** Sent from SoldierWeapon, shows NO AMMO text. */
	 ...
	 */
	void SetMatchState(ESoldierMatchState::Type NewState);

	/** Get state of current match. */
	ESoldierMatchState::Type GetMatchState() const;
...
protected:
	...
	/** State of match. */
	ESoldierMatchState::Type MatchState;

	/** Is the scoreboard widget on screen? */
	uint32 bIsScoreBoardVisible:1;

	/** Scoreboard widget. */
	TSharedPtr<class SSoldierScoreboardWidget>	ScoreboardWidget;
